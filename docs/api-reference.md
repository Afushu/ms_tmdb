...
4. 当系统设置中关闭 `local_write_enabled` 后，仍会优先读取已有本地数据，但回源成功后不会新增或更新本地库。
...
- `proxy_url`：后端访问 TMDB 时使用的网络代理地址，空字符串表示直连。
- `local_write_enabled`：是否允许代理回源成功后自动写入本地库；关闭后仍会读取已有本地数据。
