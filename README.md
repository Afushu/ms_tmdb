...
项目目标：
- 统一代理 TMDB v3 API；后端原生兼容 `/api/v3/*`、`/api/tmdb/*`、`/v3/*`、`/3/*`。
...
- 本地缓存与回源：详情请求优先走本地，过期后自动回源；可在系统设置关闭代理自动写入本地库。
...
- 前端页面：
  ...
  - `/tv/:id` 剧集详情
- 后端接口：
  - `/api/v3/*` TMDB 代理主入口（推荐）
  - `/api/tmdb/*`、`/v3/*`、`/3/*` TMDB 代理兼容入口
  - `/api/admin/*` 本地管理接口
...
- 服务地址：直连后端 `http://localhost:8888`，经 nginx / Docker 默认 `http://localhost:8080`
- TMDB 代理入口（后端原生兼容，无需依赖网关改写）：
  - 推荐：`/api/v3/*`
  - 兼容：`/api/tmdb/*`、`/v3/*`、`/3/*`
- 返回：`application/json`
...
- `/api/v3/movie/{id}`、`/api/v3/tv/{id}`、`/api/v3/person/{id}` 都支持同类调用；也可使用 `/api/tmdb/*`、`/v3/*`、`/3/*` 等价路径。
